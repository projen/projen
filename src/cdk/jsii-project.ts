...
export class JsiiProject extends TypeScriptProject {
  ...
  constructor(options: JsiiProjectOptions) {
    ...
    if (options.releaseToNpm != false) {
      ...
      this.addTargetToBuild(
        "js",
        this.packageJsTask,
        extraJobOptions,
        options.codeArtifactOptions,
      );
      this.addTargetToRelease(
        "js",
        this.packageJsTask,
        npmjs,
        options.codeArtifactOptions,
    ...
    if (maven) {
      ...
      this.addTargetToRelease("java", task, maven, options.codeArtifactOptions);
    }
    ...
    if (pypi) {
      ...
      this.addTargetToBuild(
        ...
        task,
        extraJobOptions,
        options.publishToPypi?.codeArtifactOptions ??
          options.codeArtifactOptions,
      );
      this.addTargetToRelease(
        "python",
    ...
    if (golang) {
      ...
      this.addTargetToRelease("go", task, golang, options.codeArtifactOptions);
    }
   ...
   * @param language Target language to add to the release workflow
   * @param packTask The packaging task for the target language, which will be a dependency of the release job
   * @param target The publishing configuration for the target language, which will be used to generate the publish steps in the release workflow
   * @param codeArtifactOptions Optional configuration for publishing to AWS CodeArtifact, used to generate additional steps for authenticating with CodeArtifact in the release workflow
   * @returns
   */
  private addTargetToRelease(
    ...
    const pacmak = this.pacmakForLanguage(
      language,
      packTask,
      codeArtifactOptions,
    );
   ...
   * @param language Target language to add to the build workflow
   * @param packTask The packaging task for the target language, which will be a dependency of the build job
   * @param extraJobOptions Additional job options such as runsOn or container that should be added to the build job
   * @param codeArtifactOptions Optional configuration for publishing to AWS CodeArtifact, used to generate additional steps for authenticating with CodeArtifact in the build workflow
   * @returns
   */
