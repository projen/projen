  ...
  constructor(options: JsiiProjectOptions) {
    ...
    if (options.releaseToNpm != false) {
      this.release?.publisher.publishToNpm({
        prePublishSteps: renderJsiiPacmakSteps('js'),
        registry: this.package.npmRegistry,
        npmTokenSecret: this.package.npmTokenSecret,
      });

      this.addPackJobToBuild('js', []);
    }
    ...
    if (options.publishToMaven) {
      ...
      this.release?.publisher.publishToMaven({
        prePublishSteps: renderJsiiPacmakSteps('java'),
        ...options.publishToMaven,
      });

      this.addPackJobToBuild('java', [
    ...
    if (pypi) {
      ...
      this.release?.publisher.publishToPyPi({
        prePublishSteps: renderJsiiPacmakSteps('python'),
        ...pypi,
      });

      this.addPackJobToBuild('python', [
    ...
    if (nuget) {
      ...
      this.release?.publisher.publishToNuget({
        prePublishSteps: renderJsiiPacmakSteps('dotnet'),
        ...nuget,
      });

      this.addPackJobToBuild('dotnet', [
    ...
    if (golang) {
      ...
      this.release?.publisher.publishToGo({
        prePublishSteps: renderJsiiPacmakSteps('go'),
        ...golang,
      });
...
type JsiiPacmakTarget = 'js' | 'go' | 'java' | 'python' | 'dotnet';
