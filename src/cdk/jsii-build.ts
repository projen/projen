...
export class JsiiBuild implements IMixin {
  ...
  public applyTo(project: IConstruct): void {
    ...
    // When the project owns its tsconfig, tell jsii to use it directly.
    // Otherwise fall back to letting jsii generate one from tsc settings.
    if (project.tsconfig) {
      jsii.tsconfig = project.tsconfig.fileName;
      ...
      if (options.excludeTypescript) {
        for (const pattern of options.excludeTypescript) {
          project.tsconfig.addExclude(pattern);
        }
      }
    } else {
      jsii.tsc = {
