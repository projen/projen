 ...
 */
export function renderProjenNewOptions(
  fqn: string,
  args: Record<string, any>,
  comments: NewProjectOptionHints = NewProjectOptionHints.NONE
): any {
  return {
    ...args,
...
export function renderJavaScriptOptions(opts: RenderProjectOptions) {
  ...
  const useSingleQuotes = (str: string | undefined) =>
    str?.replace(/"(.+)"/, "'$1'");
  ...
  for (const option of opts.type.options) {
    ...
    if (opts.args[optionName] !== undefined) {
      ...
      renders[optionName] = `// ${optionName}: ${useSingleQuotes(
        defaultValue
      )},`;
    }
  }
