...
class TestMatch {
  ...
  /**
   * Resolved lazily so patterns added or removed after construction are
   * still reflected.
   */
...
/**
 * The set of reporters configured for a `NodeNativeTestRunner`.
 *
 * Provides the `test.testReporter` and `test.testReporterDestination` values
 * of the generated Node.js configuration file.
 */
class NodeReporters {
  ...
  private ensureDestinationDirectory(destination: Destination): void {
    ...
    // the reporter does not create its output directory, so it must exist
...
export class NodeNativeTestRunner extends Component {
  ...
  private readonly reporters: NodeReporters;
  ...
  constructor(scope: IConstruct, options: NodeNativeTestRunnerOptions = {}) {
    ...
    // Node's snapshot files are generated, not authored.
    ...
    this.reporters = new NodeReporters(this.project);
    ...
    // Test match patterns are positional arguments and must come after every
    // flag: once Node sees the first positional argument it stops parsing
    // flags, so any flag appended after them (e.g. `--watch`) is ignored.
   ...
   * @default Destination.STDOUT
   */
  public addReporter(name: ReporterKind, destination?: Destination) {
    this.reporters.add(name, destination);
  }
   ...
   * @param name The name/kind of the reporter to remove.
   */
  public removeReporter(name: ReporterKind) {
    this.reporters.remove(name);
  }

  /**
   * Lists the configured reporters, in the order they were added.
   */
  public listReporters(): NodeReporter[] {
