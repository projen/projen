...
export class SonarqubeProperties extends Component {
  ...
  constructor(scope: IConstruct, options: SonarqubePropertiesOptions) {
    ...
    if (options.scm) {
      ...
      if (options.scm.exclusions?.disabled !== undefined) {
        sonar.scm.exclusions = {
          disabled: String(options.scm.exclusions.disabled),
        };
      }
    }
    ...
    if (options.qualitygate) {
      ...
      if (options.qualitygate.wait !== undefined) {
        qg.wait = String(options.qualitygate.wait);
      }
      if (options.qualitygate.timeout !== undefined) {
    ...
    // Extra properties (user-specified arbitrary properties). Applied as
    // overrides so a leaf like "sonar.coverage" replaces the whole subtree
    // rather than merging with the typed options above.
