...
export class TypeScriptProject extends NodeProject {
  ...
  constructor(options: TypeScriptProjectOptions) {
    ...
    if (options.eslint ?? true) {
      ...
      // eslint only lints files that are included in the tsconfig it refers to.
      // if projen isn't generating tsconfig (for example if it is generated by jsii),
      // then we can't modify the tsconfig to include additional files to lint,
      // so we create a separate tsconfig instead
      if (options.disableTsconfig && !options.eslintOptions?.tsconfigPath) {
