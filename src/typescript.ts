...
export class TypeScriptProject extends NodeProject {
  ...
  constructor(options: TypeScriptProjectOptions) {
    ...
    if (options.eslint ?? true) {
      ...
      // eslint will only lint files that are included in `tsconfig.include`.
      // if tsconfig is disabled (for example if it is generated by jsii) we cannot
      // modify the include/exclude items, so we create a separate tsconfig instead
      if (options.disableTsconfig) {
        this.eslint.generateTypescriptConfig({
