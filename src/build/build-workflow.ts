...
export class BuildWorkflow extends Component {
  ...
  /**
   * Run a command as a job within the build workflow which is executed after
   ...
   * @param options Specify tools and other options
   */
  public addPostBuildJobCommand(id: string, command: string, options?: AddPostBuildJobCommandOptions) {
    const steps = [];
    ...
    if (options?.checkoutRepo && options?.installDeps && this.project instanceof NodeProject) {
      steps.push(
        {
...
/**
 * Options for `BuildWorkflow.addPostBuildJobCommand`
 */
export interface AddPostBuildJobCommandOptions {
  ...
  /**
   * Check out the repository at the pull request branch.
   *
   * @default false
   */
  ...
  /**
   * Download build artifacts generated by the `build` job. Only runs
   * if `project.artifactsDirectory` is specified.
   *
   * @default false
   */
  readonly useArtifacts?: boolean;

  /**
   * Install any dependencies need to run tasks. `checkoutRepo` must also be
   * set to true.
   *
