...
test("multiple levels", () => {
  ...
  const child1 = new TestSubproject({
    name: "child1",
    parent: root,
    outdir: "child1",
  });
  const child2 = new TestSubproject({
    name: "child2",
    parent: child1,
    outdir: "child2",
  });
...
test("subprojects cannot introduce files that override each other", () => {
  const root = new TestProject();
  const child = new TestSubproject({
    name: "sub-project",
    parent: root,
    outdir: "sub-project",
  });
...
test('"outdir" for subprojects must be relative', () => {
  ...
  expect(
    () =>
      new TestSubproject({ name: "foobar", parent: root, outdir: "/foo/bar" })
