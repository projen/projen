...
test("fails if the step fails", () => {
  ...
  // THEN
  expect(() => executeTask(p, "test")).toThrow(
    /Task \"test\" failed when executing \"false\"/
  );
});
...
test("multiple steps", () => {
  ...
  // THEN
  expect(executeTask(p, "test")).toEqual(["step1", "step2", "step3"]);
});
...
test("execution stops if a step fails", () => {
  ...
  // THEN
  expect(() => executeTask(p, "test")).toThrow(
    /Task \"test\" failed when executing \"echo failing && false\"/
  );
});
...
function executeTask(p: Project, taskName: string) {
  ...
  const args = [require.resolve("../../cli"), taskName].map((x) => `"${x}"`);
