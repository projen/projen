...
import type { EndOfLine } from "./gitattributes";
...
import type { InitProjectOptionHints } from "./option-hints";
...
export class Project extends Construct {
  ...
  constructor(options: ProjectOptions) {
    ...
    const { gitattributes, gitignore } = initGitFiles(this, options);
  ...
  public get repo(): Repository {
    const repo = tryFindClosestRepository(this);
    if (!repo) {
      throw new Error(
        `No Repository found in the construct tree above '${this.node.path}'`,
      );
    }
    return repo;
  }
...
/**
 * Initialize git files for a project.
 *
 * For root projects under a GitRepository, the repository owns the git files
 * but they are created on the project (FileBase requires a Project scope).
 * For subprojects or projects without a GitRepository, files are created directly.
 */
function initGitFiles(
  project: Project,
  options: ProjectOptions,
): { gitattributes: GitAttributesFile; gitignore: IgnoreFile } {
  const repo = tryFindClosestRepository(project);
  const gitRepo = repo && "_initGitFiles" in repo ? (repo as any) : undefined;

  if (gitRepo && !options.parent) {
    // Root project under a GitRepository — let the repository initialize git files on us
    gitRepo._initGitFiles(project);
    return {
      gitattributes: gitRepo.gitattributes,
      gitignore: gitRepo.gitignore,
    };
  }

  // Subproject or no GitRepository — create git files directly (original behavior)
  ...
  return { gitattributes, gitignore };
}
