...
export class Artifacts {
  ...
  public static resolve(startDir: string = __dirname): Artifacts {
    ...
    return new Artifacts(path.join(repoRoot, "dist"), repoRoot);
  }
   ...
   *
   * Read from the repo's `package.json` (projen sets `version` there before
   * packaging, and it is `0.0.0` for dev/PR builds). This is language-agnostic,
   * so it works in jobs that only build one language's package (and therefore
   * have no npm tarball to parse). When resolved via `PROJEN_DIST_DIR` (no repo
   * root), it falls back to parsing the npm tarball filename, handling both
   * npm's `projen-<version>.tgz` and jsii-pacmak's `projen@<version>.jsii.tgz`.
   */
  public get version(): string {
    if (this.repoRoot) {
      const pkg = path.join(this.repoRoot, "package.json");
