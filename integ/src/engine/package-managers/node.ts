...
export function runProjenCli(
  ...
  return run("node", [cli, ...args], {
    ...
    env: {
      ...
      // built version.
      NODE_ENV: undefined,
      // `projen new` runs `git init` + `git commit`; CI runners have no global
      // git identity. Supply one via env (portable, no global config mutation).
      GIT_AUTHOR_NAME: "projen-integ",
      GIT_AUTHOR_EMAIL: "projen-integ@example.com",
      GIT_COMMITTER_NAME: "projen-integ",
      GIT_COMMITTER_EMAIL: "projen-integ@example.com",
      ...options.env,
    },
  });
}
