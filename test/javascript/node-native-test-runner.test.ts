...
test("removeReporter() removes a configured reporter", () => {
  const project = newProject();
  const nodeNativeTestRunner = new NodeNativeTestRunner(project);

  expect(nodeNativeTestRunner.listReporters().map((r) => r.name)).toContain(
    "junit",
  );

  nodeNativeTestRunner.removeReporter("junit");

  expect(nodeNativeTestRunner.listReporters().map((r) => r.name)).not.toContain(
    "junit",
  );
});
