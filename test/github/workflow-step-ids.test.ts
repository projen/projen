 ...
 * - A user can always find any step by its id via getJob().
 */
...
describe("step id assignment", () => {
  ...
  test("duplicate name-derived ids get _2, _3 suffixes", () => {
    ...
    const job = workflow.getJob("build") as Job;
    expect(job.steps[0].id).toBe("run_tests");
    expect(job.steps[1].id).toBe("run_tests_2");
    expect(job.steps[2].id).toBe("run_tests_3");
  });
  ...
  test("all steps are referenceable by id via getJob immediately after addJob", () => {
    const project = new TestProject();
    const workflow = new GithubWorkflow(project.github!, workflowName);
    ...
    expect(job.steps.find((s) => s.id === "explicit")).toBeDefined();
    expect(job.steps.find((s) => s.id === "named_step")).toBeDefined();
    expect(job.steps.find((s) => s.id!.startsWith("auto_"))).toBeDefined();
  });
});
...
describe("duplicate step id disambiguation", () => {
  ...
  test("explicit id is preserved when a name-derived id would collide", () => {
    ...
    const job = workflow.getJob("build") as Job;
    const explicitStep = job.steps.find((s) => s.id === "checkout");
    expect(explicitStep).toBeDefined();
    expect(explicitStep!.run).toBe("git checkout main");
...
describe("step id reference integrity", () => {
  ...
  test("job outputs can reference explicit step ids even when names collide", () => {
    ...
    const job = workflow.getJob("build") as Job;
    const step = job.steps.find((s) => s.id === STEP_ID);
  ...
  test("inline if expressions can reference explicit step ids even when names collide", () => {
    ...
    const job = workflow.getJob("release") as Job;
    const producer = job.steps.find((s) => s.id === STEP_ID);
    expect(producer).toBeDefined();
    expect(producer!.name).toBe("Check for mutations");

    const consumer = job.steps.find((s) => s.name === "Upload patch");
