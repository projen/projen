...
describe("lambda functions", () => {
  test("are auto-discovered by default", () => {
    ...
    expect(
      ...
      {
        exec: 'esbuild --bundle src/my.lambda.ts --target="node22" --platform="node" --outfile="resources/my.lambda/index.js" --tsconfig="test/tsconfig.json" --external:foo --external:bar --sourcemap',
      },
    ]);
  });
...
describe("workflow container image", () => {
  it("does not use an image by default for cdk v1", () => {
    const project = new TestProject({ cdkVersion: "2.189.1" });
    const snapshot = synthSnapshot(project);
    const buildWorkflow = YAML.parse(snapshot[".github/workflows/build.yml"]);
