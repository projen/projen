...
test("upgrade task ignores pinned versions", () => {
  ...
  expect(tasks.upgrade.steps).toMatchInlineSnapshot(`
   [
     {
       "exec": "npx npm-check-updates@18 --upgrade --target=minor --peer --no-deprecated --dep=dev,peer,prod,optional --filter=eslint-import-resolver-typescript,eslint-plugin-import,jest,projen",
     },
     {
