import { Project, Testing, TextFile } from "../src";
...
test("empty file", () => {
  ...
  // THEN
  const output = Testing.synth(project)["hello/foo.txt"];
...
test("initialized with some lines", () => {
  ...
  // THEN
  const output = Testing.synth(project)["boom/boom/bam.txt"];
...
test("addLine() can add lines later", () => {
  ...
  // THEN
  const output = Testing.synth(project)["hello-world.txt"];
